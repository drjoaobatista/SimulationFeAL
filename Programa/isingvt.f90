...
subroutine IsingBCC(resultado, sigma, L, MCx, MCc,J1,J2,tin,tfi,npontos, p,aglomeradolimite)
    implicit none
    !saidas
    integer, intent(out),dimension(0:npontos-1,0:2):: resultado
    integer, intent(out),dimension(1:2,1:L,1:L,1:L):: sigma
!f2py intent(out) :: resultado
    !entradas
    integer, intent(in)::L, MCx, MCc, aglomeradolimite ,nPontos! para que tamanho de agloreado exixte j2
    !f2py intent(in) ::L, MCx, MCc,aglomeradolimite, npontos
    real(8), intent(in)::J2,J1,tin,tfi, p   !p=0 sistema puro
